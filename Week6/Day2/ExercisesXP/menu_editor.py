...
def remove_item_from_menu():
    ...
    item = MenuManager.get_by_name(user_input)
    if item:
        try:
            item.delete()
            print('Item deleted successfully')
        except (psycopg2.errors.IntegrityError, psycopg2.errors.ProgrammingError, psycopg2.errors.OperationalError):
            print("We couldn't remove this item from the database, contact your database manager for more information or try again later.")
    else:
...
def update_item_from_menu():
    ...
    item_from_db = MenuManager.get_by_name(item_name)
...
- Item Name & Item Price (NP)\n').upper()
        ...
        if option_update == 'N':
            ...
            try:
                item_from_db.update(new_name = name)
       ...
       # Update price only 
            ...
            try:
                item_from_db.update(new_price=price)
                print('Item price updated successfully')
            except (psycopg2.errors.IntegrityError, psycopg2.errors.ProgrammingError, psycopg2.errors.OperationalError):
                    print("We couldn't add your item to the database, contact your database manager for more information or try again later.")
            ...
            try:
                item_from_db.update(new_name=name, new_price=price)
                print('Item price updated successfully')
            except (psycopg2.errors.IntegrityError, psycopg2.errors.ProgrammingError, psycopg2.errors.OperationalError):
                print("We couldn't add your item to the database, contact your database manager for more information or try again later.")
...
def show_restaurant_menu():
    ...
    restaurant_menu = MenuManager.all_items()
